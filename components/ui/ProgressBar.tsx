...
export const ProgressBar: React.FC<ProgressBarProps> = ({ min, max, currentValue, additionalValue = 0, className, ...props }) => {
  return (
    <div 
      ...
      {...props}
    >
      <div style={{ width: `${((currentValue + additionalValue) / max) * 100}%` }} className="progress-bar-shadow absolute top-0 left-0 h-full transition-all duration-300"></div>
      <div style={{ width: `${(currentValue / max) * 100}%` }} className="progress-bar-value absolute top-0 left-0 h-full transition-all duration-300"></div>
